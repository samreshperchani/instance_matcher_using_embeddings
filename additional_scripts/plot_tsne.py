...
def tsne_plot(model, word_1, word_2):
    ...
    labels.append(word_1.replace('http://dbkwik.webdatacommons.org/',''))

    tokens.append(model.wv[word_2])
    labels.append(word_2.replace('http://dbkwik.webdatacommons.org/',''))

    for neighbour in model.wv.most_similar(word_1, topn= count_of_neighbours):
        tokens.append(model.wv[neighbour[0]])
        labels.append(neighbour[0].replace('http://dbkwik.webdatacommons.org/',''))
    ...
    for i in range(len(x)):
        plt.scatter(x[i],y[i], c = 'g' if str(labels[i]).find('memory_alpha')!=-1 else 'b')
        plt.annotate(labels[i],
                     xy=(x[i], y[i]),
                     xytext=(5, 2),
                     textcoords='offset points',
                     ha= 'left' if str(labels[i]).find('memory_alpha')!=-1 else 'right',
                     va='bottom')
    #plt.show()
    green_circle = mlines.Line2D([], [], color='green', marker='c', linestyle='None',
                          markersize=10, label='memory-alpha')
    blue_circle = mlines.Line2D([], [], color='blue', marker='c', linestyle='None',
